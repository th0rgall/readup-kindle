...
import { isValidContent, createMetadataElements } from "./figureContent.ts";
...
function prune(
  element: ChildNode,
  depth: number,
  isInsideImageContainer: boolean,
  content: Node[][],
  ...
  if (
    ...
    !(
      isInsideImageContainer &&
      isElement(element) &&
      isValidContent(element, config)
    )
  ) {
    element.remove();
  } else {
    const isImageContainer =
      !isInsideImageContainer && isImageContainerElement(element);
    if (isImageContainer) {
      const image = images.find((image) => image.containerElement === element);
      if (image) {
        image.containerElement.classList.add(
    ...
    Array.from(element.childNodes).forEach((child) => {
      prune(
        child,
        depth + 1,
        isImageContainer || isInsideImageContainer,
        content,
        images,
        config
      );
...
function extendLineagesToContentSearchRootElement<T extends ContentContainer>(
  ...
  return containers.map((container) => newContainer(container, lineage));
}
export default function pruneDocument(parseResult: ParseResult) {
  ...
  const imageContainers = extendLineagesToContentSearchRootElement(
    ...
    (container, extendedLineage) =>
      new ImageContainer(
        extendedLineage.concat(container.containerLineage),
        container.contentLineages.map((contentLineage) =>
          extendedLineage.concat(contentLineage)
        ),
        container.caption,
        container.credit
      )
  ...
  const whitelistedScripts = Array.from(
    document.querySelectorAll("body script")
  ).filter((script) =>
    whitelistedScriptTypes.includes((script as HTMLScriptElement).type)
  );
  if (whitelistedScripts.length) {
    Array.from(whitelistedScripts).forEach((script) => {
      buildLineage({
        descendant: script.hasChildNodes() ? script.childNodes[0] : script,
        ancestor: document.body,
  ...
  if (document.body.children.length === 1) {
    ...
    while (
      ...
      !parseResult.primaryTextContainers.some(
        (container) => container.containerElement === contentRoot
      )
    ) {
