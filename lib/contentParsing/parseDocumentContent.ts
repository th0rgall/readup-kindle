...
import { isValidContent as isValidFigureContent } from "./figureContent.ts";
import Config from "./configuration/Config.ts";
import ContainerSearchConfig from "./configuration/ContainerSearchConfig.ts";
import ImageContainerMetadataConfig from "./configuration/ImageContainerMetadataConfig.ts";
import TextContainerSelectionConfig from "./configuration/TextContainerSelectionConfig.ts";
import TextContainerSearchConfig from "./configuration/TextContainerSearchConfig.ts";
import ImageContainerContentConfig from "./configuration/ImageContainerContentConfig.ts";
import configs from "./configuration/configs.ts";
import { findPublisherConfig } from "./configuration/PublisherConfig.ts";
  ...
  function addImage(
    ...
    if (
      ...
      if (!imgElements.length || validImgElements.length) {
        ...
        const metaSearchRoot = (searchUpLineage(lineage, (ancestor, index) => {
          if (index === 0) {
            return false;
          }
...
function findChildren(
  ...
  if (edge !== GraphEdge.None && depth < searchArea.length - 1) {
    ...
    if (edge & GraphEdge.Right) {
      lastSearchableChildIndex = children.length -
        1 -
        children
          .reverse()
          .findIndex((child) =>
...
export default function parseDocumentContent(
  ...
  let primaryTextContainerSearchResults = selectedPreferredPathHopCountGroups
  ...
  const primaryTextRootNode = primaryTextContainerSearchResults[0].textContainer
    .containerLineage[
      primaryTextContainerSearchResults[0].textContainer.containerLineage
        .length -
      Math.max(
        Math.max(
