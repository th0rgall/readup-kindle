...
import parseElementMicrodata from "./parseElementMicrodata.ts";
import parseSchema from "./parseSchema.ts";
import parseMiscMetadata from "./parseMiscMetadata.ts";
import parseOpenGraph from "./parseOpenGraph.ts";
import { ParserDocumentLocation } from "./ParserDocumentLocation.ts";
...
function first<T>(
  propSelector: (result: ParseResult) => T,
  results: ParseResult[],
): T;
function first<T>(
  propSelector: (result: ParseResult) => T,
...
function first<T>(
  propSelector: (result: ParseResult) => T,
  filterOrResults: ((value: T) => boolean) | ParseResult[],
  results?: ParseResult[],
) {
...
export default function parseDocumentMetadata(params: ParserParams) {
  ...
  if (
    Array.from(document.getElementsByTagName("article")).filter(
      (element) =>
        !(
  ...
  if (script && script.textContent) {
    const cdataMatch = script.textContent.match(
      /^\s*\/\/<!\[CDATA\[([\s\S]*)\/\/\]\]>\s*$/,
    );
    try {
      if (cdataMatch) {
  ...
  if (schema) {
    ...
    (schema = parseSchema(parseElementMicrodata(document.documentElement)))
  ) {
    isArticle = true;
  } else {
    schema = emptyResult;
