...
export default class ContentElement {
  ...
  private isLineReadable(line: Line) {
    return (
      this._contentRect.top + line.top >= window.pageYOffset &&
      this._contentRect.top + line.top <=
  ...
  private _setLines(readState: ReadState) {
    var lineCount = Math.max(
        1,
        Math.floor(this._contentRect.height / this._lineHeight),
      ),
    ...
    for (var i = 0, wordCount = 0; i < lineCount; i++) {
      var lineWordCount = minWordsPerLine +
        (remainder > 0 ? remainder - --remainder : 0);
      this._lines.push(
        new Line(
          this._lineHeight * i,
          this._lineHeight,
  ...
  private _getContentRect() {
    ...
    return {
      ...
      width: rect.width -
        (this._contentOffset.left + this._contentOffset.right),
      height: rect.height -
        (this._contentOffset.top + this._contentOffset.bottom),
    };
  }
  private _syncDebugDisplay() {
    ...
    if (lineCount > debugElementCount) {
      ...
      this._debugElements
        ...
        .forEach((deletedDebugElement) => {
          deletedDebugElement.remove();
        });
    }
    // set the backgrounds
    this._lines.forEach((line, index) => {
      const debugElement = this._debugElements[index],
        percentComplete = line.readState.getPercentComplete();
      debugElement.style.top = line.top + "px";
      debugElement.style.backgroundImage =
        `linear-gradient(to right, rgba(0, 255, 0, 0.5) ${percentComplete}%, transparent ${percentComplete}%)`;
    });
  }
  ...
  public updateOffset() {
    ...
    if (
      contentRect.top !== this._contentRect.top ||
