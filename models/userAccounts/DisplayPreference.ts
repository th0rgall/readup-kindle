...
export function getDisplayPreferenceChangeMessage(
  prevPreference: DisplayPreference,
  nextPreference: DisplayPreference,
) {
  let message: string;
  if (nextPreference.hideLinks !== prevPreference.hideLinks) {
    ...
    message = `Text Size ${
      nextPreference.textSize > prevPreference.textSize
        ? "Increased"
        : "Decreased"
    }`;
  } else if (nextPreference.theme !== prevPreference.theme) {
    message = `${
      nextPreference.theme === DisplayTheme.Dark ? "Dark" : "Light"
    } Theme Enabled`;
  }
  return message;
}
